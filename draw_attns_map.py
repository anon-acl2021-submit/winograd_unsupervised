...
MODEL_CLASSES = {
    'bert-base-multilingual-uncased':   (BertTokenizer, BertForMaskedLM, '[MASK]', '##', ((9, 10), (10, 1), (7, 4), (9, 2), (0, 0))),
    'xlm-roberta-large':                (XLMRobertaTokenizer, XLMRobertaModel, '<mask>', '▁', ((18, 5), (17, 7), (18, 2), (18, 10), (12, 6))),
}
...
def calc_attn_map(args, model, tokenizer, lang, pronoun_pos, sentence_a):
    ...
    boost = 2.
    for pos, token in enumerate(tokens):
        if token == args.token_prefix: continue
        if token in ('[CLS]', '[SEP]', '<s>','</s>'): continue
        ...
        if pos == pronoun_pos[0]: html_color = '#c0c0c0'
        res_html.append( f'<span style="background-color:{html_color}">{token.replace("<","&lt;").replace(">","&gt;")}</span>' )
    if lang in ('zh','jp'):
