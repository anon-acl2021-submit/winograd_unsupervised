...
from utils import (clean_sentence, tokenize, locate,
                locate_reference, list_positions,
                simple_detokenizer, get_nltk_detokenizer,
                format_attention)
...
def MAS_patched(args, model, tokenizer, pronoun_pos, candidate_a_pos, candidate_b_pos, sentence_a, sentence_b=None, layer=None, head=None, method='sum'):
    """
    Computes the Maximum Attention Score (MAS) given a sentence, a pronoun and candidates for substitution.
    Parameters
    ----------
...
def main():
    ...
    parser.add_argument("--input_file", default='dataset.nltk_tokenized.new.tsv', type=str, #required=True,
    ...
    with open(args.input_file, encoding='utf-8') as ifh:
        for line in ifh:
            ...
            if not ref_locations:
                ref_tokens, ref_locations = locate_reference(args, tokenizer, ref[0], sent_tokens, sentence, method='patch')
            if not ref_locations:
                ref_tokens, ref_locations = locate_reference(args, tokenizer, ref[0], sent_tokens, sentence, method='approx')
            ...
            for decoy in decoys:
                try:
                    sc = MAS(args, model, tokenizer, pronoun=ref[0], candidate_a=right_answer[0], candidate_b=decoy[0], sentence_a=sentence_orig)
                    if sc[0]>sc[1]:
                        stats[lang][1] += 1
                ...
                try:
                    ...
                    sc = MAS_patched(args, model, tokenizer, pronoun_pos=ref_locations, candidate_a_pos=right_answer_positions, candidate_b_pos=decoy_answer_positions, sentence_a=sentence, method='max')
                    if sc[0]>sc[1]:
    ...
    for k in sorted(stats):
        print(f"{k}\t{stats[k][1]/max(1,stats[k]['__total__'])}\t{stats[k]['__total__']}\t{stats[k]['__skipped__']}")
    print('\n====== MAS_patched_score:')
    for k in sorted(stats_patched):
        print(f"{k}\t{stats_patched[k][1]/max(1,stats_patched[k]['__total__'])}\t{stats_patched[k]['__total__']}\t{stats_patched[k]['__skipped__']}")
