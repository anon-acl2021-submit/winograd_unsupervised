...
def base1(args, model, tokenizer, sent_tokens, loc, right_answer_tokens):
    ...
    for pos in range(len(unmasked)):
        if pos in (0,len(unmasked)-1): continue
...
def main():
    ...
    with open(args.input_file, encoding='utf-8') as ifh:
        for line in ifh:
            ...
            for decoy in decoys:
                ...
                decoy_answer_raw_score1 = base1(args, model, tokenizer, sent_tokens, ref_locations[0], decoy_answer_tokens)
                decoy_answer_raw_score2 = base2(args, model, tokenizer, sent_tokens, ref_locations[0], decoy_answer_tokens)
                decoy_answer_raw_score3 = base3(args, model, tokenizer, sent_tokens, ref_locations[0], decoy_answer_tokens)
    ...
    for baseline in ('1', '2', '3'):
        for normalizer in ('raw', 'tok', 'word'):
            print(f'\n====== Baseline {baseline} with "{normalizer}" normalizer:')
            for k in sorted(stats):
                print(f"{k}\t{stats[k][f'__{baseline}{normalizer}__']/max(1,stats[k]['__total__'])}\t{stats[k]['__total__']}")
